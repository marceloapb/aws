...
router.post('/', validateToken, async (req, res) => {
  try {
    ...
    const orcamentoItem = {
      ...
      locais: locais || [],
      // Compatibilidade: primeiro local também nos campos legados
      local_nome: locais?.[0]?.nome || local_nome || '',
      local_cep: (locais?.[0]?.cep || local_cep || '').replace(/\D/g, ''),
      local_logradouro: locais?.[0]?.logradouro || local_logradouro || '',
      local_numero: locais?.[0]?.numero || local_numero || '',
      local_complemento: locais?.[0]?.complemento || local_complemento || '',
      local_bairro: locais?.[0]?.bairro || local_bairro || '',
      local_cidade: locais?.[0]?.cidade || local_cidade || '',
      local_uf: locais?.[0]?.uf || local_uf || '',
    ...
    try {
      await whatsapp.enviarTemplate({
        ...
        header_image_url: require('../services/whatsappTemplateCache').getTemplateImageUrl('mbf_boas_vindas_img'),
        parameters: [
      ...
      if (adminPhone) {
        try {
          await whatsapp.enviarTemplate({
            ...
            header_image_url: require('../services/whatsappTemplateCache').getTemplateImageUrl('mbf_notificacao_geral_img'),
            parameters: [
