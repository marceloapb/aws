...
export default function AlbumDetalhe() {
  ...
  const fetchAlbum = useCallback(async () => {
    try {
      ...
      try { const tr = await authFetch(`/admin/albuns/${id}/tema`); const tj = await tr.json(); if (tj.success && tj.data) { setTema(t => ({ ...t, ...tj.data })); setTextoAlbum(t => ({ ...t, texto_botao: tj.data.texto_botao || t.texto_botao || 'Ver fotos', mais_info: tj.data.mais_info ?? t.mais_info ?? true, info_negocio: tj.data.info_negocio ?? t.info_negocio ?? true, nome_negocio: tj.data.nome_negocio || t.nome_negocio || '' })); } } catch {}
    } catch {}
  }, [id, authFetch]);
            ...
            <div className="p-4">
              ...
              {textoTab === 'capa' && (
                <div className="space-y-4">
                  ...
                  <div><label className="text-xs font-medium text-gray-700">Texto do botão</label><input value={textoAlbum.texto_botao} onChange={e => setTextoAlbum({ ...textoAlbum, texto_botao: e.target.value })} onBlur={saveTexto} className="w-full mt-1 px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-200 outline-none" /></div>
                  <div className="flex items-center justify-between"><label className="text-xs font-medium text-gray-700">Mais informações</label><button onClick={() => { const v = !textoAlbum.mais_info; setTextoAlbum({ ...textoAlbum, mais_info: v }); const t = { ...tema, mais_info: v }; setTema(t); saveTema(t); }} className={`w-10 h-5 rounded-full relative transition-colors ${textoAlbum.mais_info ? 'bg-blue-500' : 'bg-gray-300'}`}><div className={`w-4 h-4 bg-white rounded-full shadow absolute top-0.5 transition-transform ${textoAlbum.mais_info ? 'translate-x-5' : 'translate-x-0.5'}`} /></button></div>
                  ...
                  <div className="flex items-center justify-between"><label className="text-xs font-medium text-gray-700">Informações do negócio</label><button onClick={() => { const v = !textoAlbum.info_negocio; setTextoAlbum({ ...textoAlbum, info_negocio: v }); const t = { ...tema, info_negocio: v }; setTema(t); saveTema(t); }} className={`w-10 h-5 rounded-full relative transition-colors ${textoAlbum.info_negocio ? 'bg-blue-500' : 'bg-gray-300'}`}><div className={`w-4 h-4 bg-white rounded-full shadow absolute top-0.5 transition-transform ${textoAlbum.info_negocio ? 'translate-x-5' : 'translate-x-0.5'}`} /></button></div>
                  ...
                  {tema.capa_layout === 'split' ? (
                    ...
                    <div className="flex flex-col h-full min-h-[400px]">
                      <div className="px-8 py-6 border-b" style={{ backgroundColor: tema.cores_capa?.overlay || '#121212', color: tema.cores_capa?.texto || '#FFFFFF', borderColor: `${tema.cores_capa?.texto || '#FFFFFF'}15` }}>
                        {textoAlbum.info_negocio && textoAlbum.nome_negocio && (
                          <p className="text-sm opacity-50 mb-1 tracking-widest uppercase" style={{ fontFamily: tema.fonte_corpo }}>{textoAlbum.nome_negocio}</p>
                        )}
                        <h1 className="text-4xl font-bold" style={{ fontFamily: tema.fonte_titulo }}>{textoAlbum.titulo || 'Título do álbum'}</h1>
                        <div className="flex items-center justify-between mt-3">
                          {textoAlbum.mais_info && textoAlbum.data_evento && (
                            <p className="text-base opacity-60" style={{ fontFamily: tema.fonte_corpo }}>{textoAlbum.data_evento}</p>
                          )}
                          <span className="text-sm opacity-60 inline-flex items-center gap-1 ml-auto" style={{ fontFamily: tema.fonte_corpo }}>{textoAlbum.texto_botao || 'Ver fotos'} →</span>
                        </div>
                      </div>
