          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center">
              ...
              {album.titulo !== galeriaNome && (
                <p className="text-base opacity-50">{album.titulo}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-3 z-10">
            <button onClick={() => { const url = `${window.location.origin}/album/${album.slug || album.id}`; if (navigator.share) { navigator.share({ title: album.titulo, url }); } else { navigator.clipboard.writeText(url); } }} className="flex items-center gap-1 text-sm opacity-60 hover:opacity-100 transition-opacity">
              <Share2 size={16} />
