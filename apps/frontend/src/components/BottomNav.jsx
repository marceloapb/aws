    ...
    <>
      ...
      {moreOpen && (
        <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setMoreOpen(false)}>
          <div className="absolute bottom-12 left-0 right-0 bg-white rounded-t-2xl p-3 pb-5 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-sm text-gray-800">Mais módulos</h3>
        ...
        <div className="flex items-center justify-around h-12">
          {mainItems.map(({ to, icon: Icon, label, end }) => (
            <NavLink
              ...
              className="flex flex-col items-center justify-center gap-0 flex-1 h-full"
